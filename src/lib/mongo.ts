...
export async function validateTeamCodeInDb(
  code: string,
  participantType?: "vit" | "external",
  participantId?: string
): Promise<{ success: boolean; teamName?: string; memberCount?: number; capacity?: number; error?: string }> {
  try {
    const db = await getMongoDb();
    ...
    const team = await db.collection("teams").findOne(
      ...
      { projection: { _id: 1, name: 1, capacity: 1, teamType: 1 } }
    );

    if (!team) return { success: false, error: "Team not found. Verify the code." };

    const teamId = team._id;
    const expectedType = participantType === "vit" ? "VIT" : participantType === "external" ? "EXTERNAL" : null;

    if (expectedType && team.teamType !== expectedType) {
      return {
        success: false,
    ...
    const collectionName = team.teamType === "VIT" ? "vit_students" : "external_students";
    const memberCount = await db.collection(collectionName).countDocuments({ teamId });
    const capacity = team.capacity || 5;

    // If participant is already a member their slot is already counted — don't block
    if (participantId && !participantId.startsWith("vit-") && !participantId.startsWith("ext-")) {
      const alreadyMember = await db.collection(collectionName).findOne(
        { _id: toObjectId(participantId), teamId },
        { projection: { _id: 1 } }
      );
    ...
    console.error("[MongoDB] validateTeamCodeInDb error:", err);
    return { success: false, error: "Could not connect to database to verify team code." };
  }
}

export async function joinTeamInDb(
  participant: { id: string; type: "vit" | "external"; userId: string | null; currentTeamId: string | null },
  code: string
): Promise<{ success: boolean; status?: "joined" | "switched" | "already-member"; error?: string }> {
  try {
    const db = await getMongoDb();
    if (!db) return { success: false, error: "Database connection unavailable." };
    ...
    const team = await db.collection("teams").findOne(
      { code: new RegExp(`^${escapeRegex(normalized)}$`, "i") },
      { projection: { _id: 1, capacity: 1, teamType: 1, name: 1 } }
    );
    if (!team) return { success: false, error: "Team not found." };

    const targetTeamId = team._id;
    const collectionName = team.teamType === "VIT" ? "vit_students" : "external_students";
    const participantCol = participant.type === "vit" ? "vit_students" : "external_students";

    // Type mismatch check
    const expectedType = participant.type === "vit" ? "VIT" : "EXTERNAL";
    if (team.teamType !== expectedType) {
      return { success: false, error: `Type mismatch: ${expectedType} participants cannot join a ${team.teamType} team.` };
    }

    // Already a member?
    let participantObjId: any;
    try { participantObjId = toObjectId(participant.id); } catch { participantObjId = participant.id; }

    const existingRecord = await db.collection(participantCol).findOne(
      { _id: participantObjId },
      { projection: { teamId: 1 } }
    );
    const currentTeamObjId = existingRecord?.teamId;

    if (currentTeamObjId && String(currentTeamObjId) === String(targetTeamId)) {
      return { success: true, status: "already-member" };
    }

    // Capacity check (excluding current member if switching)
    const memberCount = await db.collection(collectionName).countDocuments({ teamId: targetTeamId });
    if (memberCount >= (team.capacity || 5)) {
      return { success: false, error: "Team is full." };
    }

    const now = new Date();
    const switchedTeams = Boolean(currentTeamObjId);

    // Leave old team if on one
    if (currentTeamObjId) {
      await db.collection(participantCol).updateOne(
        { _id: participantObjId },
        { $set: { teamId: null, joinedAt: null, updatedAt: now } }
      );

      // Clean up empty old team
      const oldTeamId = currentTeamObjId;
      const oldVitCount = await db.collection("vit_students").countDocuments({ teamId: oldTeamId });
      const oldExtCount = await db.collection("external_students").countDocuments({ teamId: oldTeamId });
      if (oldVitCount + oldExtCount === 0) {
        await db.collection("submissions").deleteMany({ teamId: oldTeamId });
        await db.collection("teams").deleteOne({ _id: oldTeamId });
      } else if (participant.userId) {
        // If was leader, reassign leadership
        const oldTeam = await db.collection("teams").findOne({ _id: oldTeamId }, { projection: { leaderId: 1 } });
        if (oldTeam?.leaderId && String(oldTeam.leaderId) === participant.userId) {
          const newLeaderVit = await db.collection("vit_students").findOne(
            { teamId: oldTeamId, userId: { $ne: null } },
            { projection: { userId: 1 }, sort: { joinedAt: 1 } }
          );
          const newLeaderExt = await db.collection("external_students").findOne(
            { teamId: oldTeamId, userId: { $ne: null } },
            { projection: { userId: 1 }, sort: { joinedAt: 1 } }
          );
          const newLeaderId = newLeaderVit?.userId ?? newLeaderExt?.userId ?? null;
          await db.collection("teams").updateOne({ _id: oldTeamId }, { $set: { leaderId: newLeaderId, updatedAt: now } });
        }
      }
    }
    ...
    await db.collection(participantCol).updateOne(
      { _id: participantObjId },
      { $set: { teamId: targetTeamId, joinedAt: now, updatedAt: now } }
    );
    await db.collection("teams").updateOne({ _id: targetTeamId }, { $set: { updatedAt: now } });

    return { success: true, status: switchedTeams ? "switched" : "joined" };
  } catch (err) {
    console.error("[MongoDB] joinTeamInDb error:", err);
    const detail = err instanceof Error ? err.message : String(err);
    return { success: false, error: `Failed to join team: ${detail}` };
  }
}
